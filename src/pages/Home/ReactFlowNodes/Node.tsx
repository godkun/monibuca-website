...
export const defaultM7sNode = (isMobile: boolean) => [
  { id: 'm7s', type: 'group', position: { x: isMobile ? 0 : 50, y: 130 }, zIndex: -1, style: { width: isMobile ? 380 : 400, height: 120, backdropFilter: 'blur(10px)', background: "gray" }, data: {} },
  { id: 'note', type: 'default', position: { x: 0, y: 0 }, parentNode: 'm7s', data: { label: 'm7s 192.168.1.1' }, className: "annotation" }
];
...
export class FlowContext {
  ...
  pipe(...nodes: (Node | NodeContainer)[]) {
    nodes.reduce((prev: NodeContainer, curr) => prev.connect(curr instanceof NodeContainer ? curr : new NodeContainer(curr, this)), nodes[0] instanceof NodeContainer ? nodes[0] : new NodeContainer(nodes[0], this));
    return this;
  }
  set pluginState([plugins, setPlugins]: [string[], React.Dispatch<React.SetStateAction<string[]>>]) {
...
export class NodeContainer {
  ...
  constructor(public data: Node, public context: FlowContext) {
...
const ToolSelector = memo<{ container: PusherContainer; }>(function ({ container }) {
  return <select onChange={
    ...
    {Object.keys(pusherProtocol).map(type => <option value={type} selected={container.data.data.tool === type} > {type} </option>)}
  </select>;
});
...
const SourceTypeSelector = memo<{ container: PullerContainer; }>(function ({ container }) {
  return <select onChange={
    ...
    {Object.keys(sourceTypes).map(type => <option value={type} selected={container.data.data.tool === type} > {type} </option>)}
  </select>;
});
...
export class PushOutContainer extends PlayerContainer {
  changePlugin(plugin: string) {
    ...
    this.context._updatePlugins();
    this.context.config = plugin == 'm7s' ? `${this.plugin}:
  push:
    pushlist:
      ${this.stream}: ${this.url}` : `${this.plugin}:
  target:
    - host: ${this.url}`;
    this.context.setConfig?.(this.context.config);
  }
...
export class EdgeContainer {
  data: Edge;
  constructor(public source: NodeContainer, public target: NodeContainer, public context: FlowContext) {
    this.data = {
...
export const protocol2plugin = { "ws-h265": "jessica", "http-h265": "jessica", "ws-flv": "jessica", "http-flv": "hdl", "ws-raw": "jessica", flv: "record", m3u8: "record", mp4: "record", raw: "record" } as const;